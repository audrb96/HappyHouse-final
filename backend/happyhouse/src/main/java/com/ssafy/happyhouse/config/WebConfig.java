...
//				.excludePathPatterns(EXCLUDE_PATHS);// 적용 제외 경로
		registry.addInterceptor(jwtInterceptor).addPathPatterns("/article/**", "/memo/**") // 기본 적용 경로
        .excludePathPatterns(Arrays.asList("/user/confirm/**", "/article/list", "/user/login","/user/join", "/user/pwFind"));// 적용 제외 경로
	}
